...
class PasswordResetManager:
    ...
    def __init__(self, request):
        ...
        self.password_reset_url = request.build_absolute_uri('/api/v1/user/reset-password/')
    ...
    def prepare_password_reset_email(self, email):
        ...
        self.context = {
            ...
            'reset_link' : self.password_reset_url + self.encoded_token.decode('utf-8')+ '/'
        }
        self.email_body = render_to_string('password_reset_email.txt', self.context)
    ...
    def get_user_from_encoded_token(self, token):
        try:
            payload_data = jwt.decode(token,settings.SECRET_KEY,algorithm='HS256')
            return payload_data
        except :
            return None
